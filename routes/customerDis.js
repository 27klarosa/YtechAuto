...
router.get('/customerDis', ensureLoggedIn, (req, res) => {
    ...
    if (!db) { console.error('Database connection not available'); return res.status(500).send('<script>alert("Database error"); window.history.back();</script>'); }

    db.all('SELECT * FROM tickets WHERE customerEmail = ? and stat = ? ORDER BY date DESC', [email, 'complete'], (err, tickets) => {
        if (err) { console.error('Database error:', err && err.message); return res.status(500).send('<script>alert("Database error"); window.history.back();</script>'); }
