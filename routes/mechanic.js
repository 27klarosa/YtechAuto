...
// graceful GET handler for /mechanic/media to avoid 404s from legacy clients
router.get('/mechanic/media', (req, res) => {
    const db = req.app && req.app.locals && req.app.locals.db;
    if (!db) return res.status(500).json({ success: false, message: 'Database not available' });
    ...
    // fetch pictures then videos and return a predictable shape
    db.all('SELECT id, filename, originalName, relativePath, mimeType, sizeBytes, uploadDate FROM pictures WHERE ticketID = ?', [ticketId], (pErr, pictures) => {
        if (pErr) {
            console.error('mechanic/media: pictures query failed', pErr);
            return res.status(500).json({ success: false, message: 'Failed to load pictures' });
        }

        db.all('SELECT id, filename, originalName, relativePath, mimeType, sizeBytes, uploadDate FROM videos WHERE ticketID = ?', [ticketId], (vErr, videos) => {
            if (vErr) {
                console.error('mechanic/media: videos query failed', vErr);
                return res.status(500).json({ success: false, message: 'Failed to load videos' });
            }

            const normalizeRow = (r) => {
                if (!r) return null;
                const src = r.relativePath ? ('/' + String(r.relativePath).replace(/^\/+/, '')) : null;
                return {
                    id: r.id,
            ...
            const images = Array.isArray(pictures) ? pictures.map(normalizeRow).filter(Boolean) : [];
            const vids = Array.isArray(videos) ? videos.map(normalizeRow).filter(Boolean) : [];
...
// graceful POST handler for /mechanic/media to support clients that send JSON { ticketId }
router.post('/mechanic/media', (req, res) => {
    const db = req.app && req.app.locals && req.app.locals.db;
    if (!db) return res.status(500).json({ success: false, message: 'Database not available' });

    const ticketId = (req.body && (req.body.ticketId || req.body.id || req.body.ticketID)) || req.query.ticketId || req.query.id;
    if (!ticketId) return res.status(400).json({ success: false, message: 'Missing ticketId' });

    db.all('SELECT id, filename, originalName, relativePath, mimeType, sizeBytes, uploadDate FROM pictures WHERE ticketID = ?', [ticketId], (pErr, pictures) => {
        if (pErr) {
            console.error('mechanic/media (POST): pictures query failed', pErr);
            return res.status(500).json({ success: false, message: 'Failed to load pictures' });
        }

        db.all('SELECT id, filename, originalName, relativePath, mimeType, sizeBytes, uploadDate FROM videos WHERE ticketID = ?', [ticketId], (vErr, videos) => {
            if (vErr) {
                console.error('mechanic/media (POST): videos query failed', vErr);
                return res.status(500).json({ success: false, message: 'Failed to load videos' });
            }
